(() => {
  ...
  const CONFIG = Object.freeze({
    defaults: { keepMobile: false, orientation: 'auto', movement: 'tilt', sensitivity: 'medium', deadzone: 'medium', invertY: 'no', touchMode: 'auto', buttonSize: 'small', opacity: '45' },
  ...
  let settings = loadSettings(); const detectedMobile = capabilities.touch || capabilities.coarse, capable = detectedMobile || settings.keepMobile;
  let names = loadNames(), sensorPermission = 'idle', sensorListening = false, sensorValidAt = 0, sensorStartedAt = 0, lastSensorEvent = 0, center = null, filtered = { x: 0, y: 0 }, joystick = { x: 0, y: 0, pointer: null }, fallbackNotified = false, lastPhysicalOrientation = null;
  ...
  function selectedGameVisible() { return capable && !document.querySelector('.screen.visible') && ['playing', 'intro', 'levelclear'].includes(window.__voidRunnerTest?.mode?.()); }
  function effectiveMovement() { return settings.movement === 'tilt' && sensorPermission === 'granted' ? 'tilt' : 'joystick'; }
  function touchWanted() { return settings.touchMode === 'on' || (settings.touchMode === 'auto' && (capabilities.touch || capabilities.coarse)); }
  function applySettings() {
    document.body.classList.toggle('mobile-capable', capable); document.body.dataset.mobileMovement = effectiveMovement(); document.body.dataset.mobileButtons = settings.buttonSize; document.documentElement.style.setProperty('--touch-opacity', String(Number(settings.opacity) / 100));
    ...
    $('#mobileSettings').hidden = !capable; $('#mobileKeepMode').checked = Boolean(settings.keepMobile); $('#touchControls').classList.toggle('visible', capable && selectedGameVisible() && touchWanted()); $('#touchJoystick').hidden = effectiveMovement() !== 'joystick';
    if (settings.movement !== 'tilt') stopSensor(); else syncSensorLifecycle(); updateOrientation();
  }
  function saveOptions() { saveNames(); if (!capable) return; settings.keepMobile = $('#mobileKeepMode').checked; for (const [id, key] of [['mobileOrientation', 'orientation'], ['mobileMovement', 'movement'], ['mobileSensitivity', 'sensitivity'], ['mobileDeadzone', 'deadzone'], ['mobileInvertY', 'invertY'], ['touchMode', 'touchMode'], ['mobileButtonSize', 'buttonSize'], ['mobileOpacity', 'opacity']]) settings[key] = $(`#${id}`).value; saveSettings(); applySettings(); }
  ...
  async function requestSensor() {
    if (!isSecureContext && !['localhost', '127.0.0.1'].includes(location.hostname)) return useJoystick('El sensor requiere HTTPS. Usando joystick táctil.');
  ...
  function getVector() {
    ...
    const now = performance.now(); if (sensorPermission !== 'granted') return { active: true, x: joystick.x, y: joystick.y };
  ...
  async function lockOrientation() { if (!document.fullscreenElement || !screen.orientation?.lock) return; const target = settings.orientation === 'auto' ? (physicalOrientation() === 'landscape' ? 'landscape' : null) : settings.orientation; if (!target) return; try { await screen.orientation.lock(target); } catch (_) { toast(`Gira el dispositivo para jugar en ${target === 'landscape' ? 'horizontal' : 'vertical'}`); } }
  ...
  $('#mobileFullscreenButton').addEventListener('click', enterFullscreen); $('#mobileSensorButton').addEventListener('click', requestSensor); $('#mobileCalibrateButton').addEventListener('click', () => calibrate(true)); $('#mobileResetButton').addEventListener('click', resetMobile); $('#mobileSaveButton').addEventListener('click', () => { saveOptions(); dispatchEvent(new CustomEvent('void-show-game-screen', { detail: 'start' })); });
  ...
  addEventListener('void-screen-change', event => { syncSensorLifecycle(); applySettings(); if (capable && !event.detail?.id && !sessionStorage.getItem('spaceShooter.fullscreenHint')) { sessionStorage.setItem('spaceShooter.fullscreenHint', '1'); toast('Consejo: usa pantalla completa desde el menú'); } if (event.detail?.id === 'startScreen') { try { screen.orientation?.unlock?.(); } catch (_) {} } }); addEventListener('void-game-started', () => { syncSensorLifecycle(); applySettings(); }); addEventListener('void-level-started', () => { syncSensorLifecycle(); applySettings(); }); addEventListener('void-player-count', updateDevices); addEventListener('gamepadconnected', updateDevices); addEventListener('gamepaddisconnected', updateDevices); addEventListener('resize', updateOrientation); addEventListener('orientationchange', updateOrientation); screen.orientation?.addEventListener?.('change', updateOrientation); document.addEventListener('fullscreenchange', onFullscreen);
  window.VoidMobile = { capable, detectedMobile, capabilities, config: CONFIG, getVector, saveNames, saveOptions, names: () => [...names], settings: () => ({ ...settings }), requestSensor, calibrate, applySettings, debug: () => ({ capable, detectedMobile, capabilities, settings: { ...settings }, names: [...names], sensorPermission, sensorListening, sensorValid: Boolean(sensorValidAt), vector: getVector(), orientation: physicalOrientation(), fullscreen: Boolean(document.fullscreenElement) }) };
})();
