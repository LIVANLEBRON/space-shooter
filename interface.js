(() => {
  ...
  async function refreshRanking() { try { const records = await window.VoidRanking.top(50), limit = window.VoidMobile?.capable ? 4 : 7; $('#topPilotsList').replaceChildren(...records.slice(0, limit).map((r, i) => rankRow(r, i, true))); $('#rankingList').replaceChildren(...records.slice(0, window.VoidMobile?.capable ? 4 : records.length).map((r, i) => rankRow(r, i))); } catch (_) { for (const id of ['topPilotsList', 'rankingList']) { const li = document.createElement('li'); li.className = 'empty-rank'; li.textContent = 'Ranking no disponible'; $(`#${id}`).replaceChildren(li); } toast('Ranking no disponible'); } }
