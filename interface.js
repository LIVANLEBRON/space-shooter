(() => {
  ...
  function rankRow(record, index, compact = false) { const li = document.createElement('li'); if (record.finalBossDefeated) li.className = 'champion'; const progress = record.finalBossDefeated ? '★ COMPLETADO' : record.highestLevel === 10 ? `N10 · JEFE ${record.finalBossProgress || 0}%` : `N${record.highestLevel}`; li.title = record.highestLevel === 10 ? `${record.finalBossProgress || 0}% de las dos vidas del jefe final superado` : `Nivel ${record.highestLevel} alcanzado`; const values = compact ? [String(index + 1), `${record.finalBossDefeated ? '★ ' : ''}${record.name}`, progress, String(record.maxScore).padStart(6, '0')] : [String(index + 1), `${record.finalBossDefeated ? '★ ' : ''}${record.name}`, progress, String(record.maxScore).padStart(6, '0'), record.playerCount === 2 ? 'COOP' : 'INDIVIDUAL']; values.forEach((value, i) => { const span = document.createElement('span'); span.textContent = value; if (compact && i === 3) span.className = 'pilot-score'; li.append(span); }); return li; }
  async function refreshRanking() { try { const records = await window.VoidRanking.top(50); if (!records.length) throw new Error('Sin récords'); const limit = window.VoidMobile?.capable ? 4 : 7; $('#topPilotsList').replaceChildren(...records.slice(0, limit).map((r, i) => rankRow(r, i, true))); $('#rankingList').replaceChildren(...records.map((r, i) => rankRow(r, i))); } catch (_) { for (const id of ['topPilotsList', 'rankingList']) { const li = document.createElement('li'); li.className = 'empty-rank'; li.textContent = 'Ranking no disponible'; $(`#${id}`).replaceChildren(li); } toast('Ranking no disponible'); } }
  ...
  window.VoidRanking.onStatus(message => { toast(message); if (message === 'Récord sincronizado') refreshRanking(); });
